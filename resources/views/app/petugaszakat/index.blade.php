                        ...
                        <div class="row justify-content-between align-items-center">
                            ...
                            <div class="col-auto">
                                <div class="card-header-action">
                                    <a href="{{ route('petugaszakat.create') }}">Tambah Data</a>
                            ...
                            <tbody>
                                @foreach ($datas as $data)
                                    <tr>
                                        ...
                                        <td class="cell text-start">{{ ucwords($data->nama) }}</td>
                                        <td class="cell">{{ $data->username }}</td>
                                        <td class="cell">
                                            <form action="{{ route('petugaszakat.update', $data->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('PUT')
                                                <button type="submit"
                                            ...
                                            <form action="{{ route('petugaszakat.destroy', $data->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    style="border-radius: 4px; padding: 0 10px 2px; margin: 5px 0;"
